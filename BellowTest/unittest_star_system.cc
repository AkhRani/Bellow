...
TEST(StarSystemTest, LoadAndSave) {
  ...
  // Nominal, no planet
  RunLua(L, "return { name = \"Sol\", pos = {x = .5, y = .6}}");
  StarSystem noplanet(game, L, 1);
  EXPECT_EQ("Sol", noplanet.m_Name);
  EXPECT_EQ(.5, noplanet.m_Position.x);
  EXPECT_EQ(.6, noplanet.m_Position.y);
  EXPECT_EQ(1, noplanet.m_ID);
  Planet& p = noplanet.GetPlanet();
  EXPECT_EQ(0, p.GetMaxPopulation());

  // Nominal, unowned planet
  RunLua(L, "none = { amount = 0, invested = 0 } return { name = \"Betel\", pos = {x = .6, y = .7}, "
  ...
  EXPECT_EQ(.6, withplanet.m_Position.x);
  EXPECT_EQ(.7, withplanet.m_Position.y);
  ...
  EXPECT_EQ(.6, loaded.m_Position.x);
  EXPECT_EQ(.7, loaded.m_Position.y);
