...
function RandomMinDistance(galaxy, minDistance)
  ...
  while attempts < 10 do
    ...
    local d = mds + 1
    for k, v in pairs(galaxy.planets) do
      local dx = v.x - x
      local dy = v.y - y
...
function NewGame(planetCount, size, minDistance)
  ...
  galaxy.size = size
  print (string.format("%d systems created", #galaxy.planets))
